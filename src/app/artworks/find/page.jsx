...
const getArtWorks = async (category) => {
    const data = await ((await fetch(`${process.env.BASE_URL}/api/artworks/arts?category=${category}`, { cache: 'no-store' })).json());
    return data['data'];
}
...
const page = async (props) => {
    ...
    return (
        <PlainLayout>
            <h1 className=' font-bold text-2xl mx-1 sm:mx-4 md:mx-8 lg:mx-20 mt-3 pb-1 text-red-500 border-b-4 border-red-200 w-fit'>Showing results for {`"${category}"`} category:</h1>
